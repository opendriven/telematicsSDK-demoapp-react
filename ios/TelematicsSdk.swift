...
@objc(TelematicsSdk)
class TelematicsSdk: RCTEventEmitter, RPLowPowerModeDelegate, RPLocationDelegate {

    private var lowPowerEventName = "onLowPowerModeEnabled"
    private var locationChangedEventName = "onLocationChanged"
    ...
    func lowPowerMode(_ state: Bool) {
        ...
        if (state == true) {
            self.sendEventIfListening(eventName: self.lowPowerEventName, body: nil)
        }
    }

    func onLocationChanged(_ location: CLLocation) {
        sendEventIfListening(eventName: self.locationChangedEventName, body: [
    ...
    @objc override func supportedEvents() -> [String] {
        return [
            locationChangedEventName,
            lowPowerEventName
            // Add any other events you want to emit
        ]
    }
